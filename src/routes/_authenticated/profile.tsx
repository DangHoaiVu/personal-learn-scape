...
export const Route = createFileRoute("/_authenticated/profile")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Cập nhật thông tin cá nhân, xem vai trò tài khoản và đăng xuất khỏi EduSense." },
...
function ProfilePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <SectionTitle title="Hồ sơ cá nhân" subtitle="Thông tin tài khoản EduSense" icon={<UserRound className="h-5 w-5" />} />
      ...
      <GlassPanel>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <button
            ...
            className="rounded-xl bg-gradient-to-r from-aurora-blue to-aurora-violet px-5 py-2.5 text-sm font-semibold text-slate-50 disabled:opacity-60"
          >
            {saving ? "Đang lưu…" : "Lưu thay đổi"}
          </button>
          <button
            onClick={signOut}
            className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-5 py-2.5 text-sm text-slate-200 transition hover:bg-white/15"
          >
