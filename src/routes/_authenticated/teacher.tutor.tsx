...
export const Route = createFileRoute("/_authenticated/teacher/tutor")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Hỏi đáp nhanh về tình hình lớp: chủ đề yếu, sinh viên nguy cơ, xu hướng điểm.",
      },
      { property: "og:title", content: "Trợ lý lớp học · EduSense" },
      {
        property: "og:description",
        content: "Tóm tắt sức khỏe lớp học từ dữ liệu năng lực và cảnh báo.",
      },
...
function TeacherTutor() {
  ...
  const { data: mastery = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const map = new Map<
        string,
        { course_id: string; topic_tag: string; sum: number; n: number }
      >();
